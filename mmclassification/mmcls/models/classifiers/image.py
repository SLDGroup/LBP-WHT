...
@CLASSIFIERS.register_module()
class IndexedImageClassifier(ImageClassifier):
    def __init__(self, backbone, neck=None, head=None, pretrained=None, train_cfg=None, init_cfg=None):
        super().__init__(backbone, neck, head, pretrained, train_cfg, init_cfg)
        self.batch_img_idx = []

    def forward_train(self, img, gt_label, **kwargs):
        self.batch_img_idx = kwargs.get('data_idx', None)
        loss = super().forward_train(img, gt_label)
        return loss
