...
class Attention4D(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        q = self.q(x).flatten(2).reshape(
            B, self.num_heads, -1, self.N).permute(0, 1, 3, 2)
        k = self.k(x).flatten(2).reshape(
            B, self.num_heads, -1, self.N).permute(0, 1, 2, 3)
        v = self.v(x)
        v_local = self.v_local(v)
        v = v.flatten(2).reshape(B, self.num_heads, -
                                 1, self.N).permute(0, 1, 3, 2)
        ...
        out = x.transpose(2, 3).reshape(
            B, self.dh, self.resolution, self.resolution) + v_local
...
class Attention4DDownsample(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        q = self.q(x).flatten(2).reshape(
            B, self.num_heads, -1, self.N2).permute(0, 1, 3, 2)
        k = self.k(x).flatten(2).reshape(
            B, self.num_heads, -1, self.N).permute(0, 1, 2, 3)
        v = self.v(x)
        v_local = self.v_local(v)
        ...
        out = x.reshape(B, self.dh, self.resolution2,
                        self.resolution2) + v_local
...
class EfficientFormerV2(BaseBackbone):
    ...
    def __init__(self,
        ...
        self.patch_embed = stem(
            in_channels, self.embed_dims[0], act_layer=act_layer)
        self.norm_eval = norm_eval

        network = []
        for i in range(len(self.layers)):
            stage = meta_blocks(self.embed_dims[i], i, self.layers,
                                ...
                                resolution=math.ceil(
                                    resolution / (2 ** (i + 2))),
                                vit_num=arch['vit_num'],
                                e_ratios=arch['expansion_ratios'][i])
        ...
        self.layer_group = [0, -1, 1, -1, 2, -1, 3]
        for i_layer in self.out_indices:
            layer = build_norm_layer(
                dict(type='BN'), self.embed_dims[i_layer])[1]

            layer_name = f'norm{i_layer}'
            self.add_module(layer_name, layer)

    def train(self, mode=True):
        super(EfficientFormerV2, self).train(mode)
    ...
    def forward_tokens(self, x):
        ...
        for idx, block in enumerate(self.network):
            x = block(x)
            if self.layer_group[idx] in self.out_indices:
                norm_layer = getattr(self, f'norm{self.layer_group[idx]}')
                x_out = norm_layer(x)
                outs.append(x_out)
        return tuple(outs)
