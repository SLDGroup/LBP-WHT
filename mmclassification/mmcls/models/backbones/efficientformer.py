...
class AttentionWithBias(BaseModule):
    ...
    def __init__(self,
        ...
        if extra_cfg.get('split_qkv', False):
            self.q = nn.Linear(embed_dims, self.nh_kd)
            self.k = nn.Linear(embed_dims, self.nh_kd)
            self.v = nn.Linear(embed_dims, self.dh)
        else:
            h = self.dh + self.nh_kd * 2
            self.qkv = nn.Linear(embed_dims, h)
        self.proj = nn.Linear(self.dh, embed_dims)
        self.extra_cfg = extra_cfg
        if extra_cfg.get('param_scale', False):
            self.scale = nn.Parameter(torch.tensor(
                [key_dim**-0.5 * 2]), requires_grad=True)
        else:
    ...
    def forward(self, x):
        ...
        """
        B, N, C = x.shape
        if self.extra_cfg.get("qk", False):
            x_qk = x.detach()
        else:
            x_qk = x
        if self.extra_cfg.get('split_qkv', False):
            q = self.q(x_qk)
            k = self.k(x_qk)
            v = self.v(x)
        ...
        qk_corr = q @ k.transpose(-2, -1)
        if self.extra_cfg.get("corr", False):
            qk_corr = qk_corr.detach()

        attn = (qk_corr * self.scale +
                (self.attention_biases[:, self.attention_bias_idxs]
                 if self.training else self.ab))
        attn = attn.softmax(dim=-1)
        if self.extra_cfg.get("attn", False):
            attn = attn.detach()
...
class Meta3D(BaseModule):
    ...
    def __init__(self,
        ...
        self.token_mixer = AttentionWithBias(
            dim, extra_cfg=extra_cfg, resolution=resolution)
