...
@DATASETS.register_module()
class CIFAR10Part(CIFAR10):
    def __init__(self, data_prefix, pipeline,
                 classes=None, ann_file=None,
                 test_mode=False, partition_idx=0, num_partition=2):
        assert partition_idx < num_partition
        self.partition_idx = partition_idx
        self.num_partition = num_partition
        super().__init__(data_prefix, pipeline, classes, ann_file, test_mode)

    def load_annotations(self):
        full_data_infos = super().load_annotations()
        start = len(self.CLASSES) // self.num_partition * self.partition_idx
        if self.partition_idx == self.num_partition - 1:
            end = len(self.CLASSES)
        else:
            end = start + len(self.CLASSES) // self.num_partition
        partition_data_infos = []
        for info in full_data_infos:
            if start <= info['gt_label'] < end:
                partition_data_infos.append(info)
        return partition_data_infos


@DATASETS.register_module()
class CIFAR100Part(CIFAR100, CIFAR10Part):
    pass
